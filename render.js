...
function viewModel(agent) {
  var venv = new Environment()
  venv.initializeGame()
  var data = venv.data
  var actions = [0, 0]
  initializeView(data)
  initializePlot(actions)
  var input_data = [arrayClone(data), arrayClone(data)]
  var done = false;
  ...
  function initialize() {
    ...
    data = venv.data;
    draw(data)
    input_data = [arrayClone(data), arrayClone(data)]
    setTimeout(update, refresh_rate)
  }

  async function update() {
    input_data = [input_data[1], arrayClone(data)]
